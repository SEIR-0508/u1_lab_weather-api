const apiKey = 'dcb319c0bda74535aa6180432231705'
...
$dropdown.on('change', async function() {
    ...
    if (raceDateFormat < today) {
        data = await axios.get(`http://api.weatherapi.com/v1/history.json?key=${apiKey}&q=${city}&dt=${raceDate}`)
        data2 = await axios.get(`https://ergast.com/api/f1/2023/${round}/results.json`)
        let results = data2.data.MRData.RaceTable.Races[0].Results[0]

        $raceWinner.text(`${results.Driver.givenName} ${results.Driver.familyName}`)
